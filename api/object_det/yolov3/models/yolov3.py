...
ANCHORS_LIST = [(10, 13), (16, 30), (33, 23), (30, 61), (62, 45),
...
class YoloLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            output = x.view(batch_size, num_anchors, num_attrib, grid_size, grid_size).permute(
                0, 1, 3, 4, 2).contiguous().view(batch_size, -1, num_attrib)
            return output
        else:
            prediction = x.view(batch_size, num_anchors, num_attrib,
                                grid_size, grid_size).permute(0, 1, 3, 4, 2).contiguous()
            self.anchors = self.anchors.to(x.device).float()
            # Calculate new Offset
            grid_tensor = torch.arange(
                grid_size, dtype=torch.float, device=x.device).repeat(grid_size, 1)
            ...
            # Get Output
            # Center x
            x_pred = (torch.sigmoid(prediction[..., 0]) + grid_x) * self.stride
            # Center y
            y_pred = (torch.sigmoid(prediction[..., 1]) + grid_y) * self.stride
            # Width
            w_pred = torch.exp(prediction[..., 2])*anchor_w
            # Height
            h_pred = torch.exp(prediction[..., 3])*anchor_h
            pred_conf = torch.sigmoid(prediction[..., 4]).view(
                batch_size, -1, 1)                # Conf
            pred_cls = torch.sigmoid(prediction[..., 5:]).view(
                batch_size, -1, self.class_num)   # Class prediction

            pred_bbox = torch.stack((x_pred, y_pred, w_pred, h_pred), dim=4).view(
                batch_size, -1, 4)      # Bounding Box (cx, cy, w, h)
...
class DetectionBlock(nn.Module):
    ...
    def __init__(self, channel_in, channel_out, mask_num, stride):
        ...
        # anchor number = 3 and numb of attribe = (class_num +5)
        self.conv7 = nn.Conv2d(channel_out, 3*(80+5), 1, bias=True)
...
class YoloV3(nn.Module):
    ...
    def yolo_last_layers(self):
        _layers = [self.yolotail.d1.conv7,
                   self.yolotail.d2.conv7, self.yolotail.d3.conv7]
        return _layers
